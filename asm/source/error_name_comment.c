...
/*   Updated: 2019/09/05 11:49:54 by sgarry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	error_name(t_pars_asm *pars_asm, t_asm **asm_list)
{
	int i;
	int j;
...
void	error_comment(t_pars_asm *pars_asm, t_asm **asm_list)
{
	int i;
	int j;
...
void	name_and_comment(t_pars_asm *pars_asm, t_asm **asm_list)
{
	...
	if (!ft_strncmp((*asm_list)->one_line,
		...
		error_comment(pars_asm, asm_list);
				...
				ft_strlen(NAME_CMD_STRING)))
		error_name(pars_asm, asm_list);
	if (!*pars_asm->name && !*pars_asm->comment)
		error_in_name_or_comment(&pars_asm, asm_list,
				"\033[31mNot name and comment in file\033[m\n");
	else if (!*pars_asm->name)
		error_in_name_or_comment(&pars_asm, asm_list,
				"\033[31mNot name in file\033[m\n");
	else if (!*pars_asm->comment)
		error_in_name_or_comment(&pars_asm, asm_list,
